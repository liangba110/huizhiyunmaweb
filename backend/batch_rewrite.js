...
(async () => {
  ...
  // 获取字数不足的文章（按字数升序，取前10篇）
  const [articles] = await db.execute(
    'SELECT id, title, category, LENGTH(content) as len FROM articles WHERE LENGTH(content) < 1500 ORDER BY LENGTH(content) ASC LIMIT 10'
  );
  ...
  for (const article of articles) {
    console.log(`\n开始重写文章 ${article.id}: ${article.title}`);
...
- H2: 总结与建议（200字）`;
    ...
    try {
      ...
      console.log(`文章 ${article.id} 重写完成:`);
      ...
      console.error(`文章 ${article.id} 重写失败:`, error.message);
      failCount++;
    }
  }
