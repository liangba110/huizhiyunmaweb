      ...
      res.on('end', () => {
        try {
          ...
          if (p.choices && p.choices[0] && p.choices[0].message) {
            ...
            reject(new Error('Invalid response: ' + JSON.stringify(p).slice(0, 200)));
          }
        } catch(e) { reject(e); }
      });
    });
...
(async () => {
  ...
  const [articles] = await db.execute(
    'SELECT id, title, category, LENGTH(content) as len FROM articles WHERE LENGTH(content) < 2000 ORDER BY LENGTH(content) ASC LIMIT 10'
  );
