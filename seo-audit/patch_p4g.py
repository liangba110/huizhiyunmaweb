...
# 在各栏目页的 seoPage('...', { 之后插入 breadcrumb
BC = [
    ("'/services',",  '开发服务', '/services'),
    ("'/cases',",     '开发案例', '/cases'),
    ("'/about',",     '关于我们', '/about'),
    ("'/templates',", '网站模板', '/templates'),
    ("'/packages',",  '服务套餐', '/packages'),
    ("'/articles',",  '行业资讯', '/articles'),
    ("'/contact',",   '联系我们', '/contact'),
]
n = 0
for i in range(start, end + 1):
    for path_lit, name, url in BC:
        pat = "seoPage(" + path_lit + " {"
        if pat in lines[i]:
            ins = " breadcrumb: [{ name: '%s', url: SITE + '%s' }]," % (name, url)
            lines[i] = lines[i].replace(pat, pat + ins, 1)
            n += 1; print('  OK [%s]' % name)
            break
